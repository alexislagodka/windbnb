...
export default function Stays () {
  ...
  const nbGuestsParam = parseInt(adultsParam) + parseInt(childrenParam)

  // Filter the stays with the parameters
  if (cityParam) stays = stays.filter((stay) => stay.city === cityParam)
  if (countryParam) { stays = stays.filter((stay) => stay.country === countryParam) }
  if (adultsParam || childrenParam) { stays = stays.filter((stay) => stay.maxGuests >= nbGuestsParam) }
